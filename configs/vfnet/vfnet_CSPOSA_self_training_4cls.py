...
data = dict(
    ...
    train=[dict(
        ...
        dict(
            type='CocoDataset',
            ann_file=
            '/usr/videodate/yehc/ImageDataSets/CrowdPose/CrowdPoseFakeLabel.json',
            img_prefix='/usr/videodate/yehc/ImageDataSets/CrowdPose/images',
            classes=['person', 'bottle', 'chair', 'potted plant'],
            pipeline=train_pipline)],
    val=dict(
        type='CocoDataset',
...
work_dir = 'work_dirs/vfnet_CSPOSA_self_training_4cls/'
load_from = None
resume_from = 'work_dirs/vfnet_CSPOSA_self_training_4cls/latest.pth'
# resume_from = None
workflow = [('train', 1)]
gpu_ids = range(0, 2)
