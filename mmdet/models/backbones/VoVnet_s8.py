...
class VoVNet_lite_s8(nn.Module):
    def __init__(self,
                 channels=[3, 16, 24, 32, 32, 32, 48, 48],
                 frozen_stages=-1,
                 norm_eval=True):
        super(VoVNet_lite_s8, self).__init__()
        ...
        self.stage2 = nn.Sequential(BottleNeck(channels[2], channels[3], kernel=3, stride=2, exp=1, group=True)
                                    )

        self.stage3 = nn.Sequential(BottleNeck(channels[3], channels[4], kernel=3, stride=2, exp=3, group=True),
                                    OSAModule(channels[4], kernel=3, exp=[1, 1, 1], layer_num=3, group=True)
                                    )

        self.stage4 = nn.Sequential(BottleNeck(channels[4], channels[5], kernel=3, stride=2, exp=6, group=True),
                                    OSAModule(channels[5], kernel=3,  exp=[1, 3, 1], layer_num=3, group=True)
                                    )

        self.stage5 = nn.Sequential(BottleNeck(channels[5], channels[6], kernel=3, stride=2, exp=6, group=True),
                                    OSAModule(channels[6], kernel=3, exp=[3, 3, 3], layer_num=3, group=True)
                                    )

        self.stage6 = nn.Sequential(BottleNeck(channels[6], channels[7], kernel=3, stride=2, exp=6, group=True),
                                    OSAModule(channels[7], kernel=3, exp=[3, 6, 3], layer_num=3, group=True)
                                    )
