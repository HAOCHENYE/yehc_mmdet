...
class OSAModule(nn.Module):
    def __init__(self, channel, kernel,exp, layer_num, group=False):
        ...
        for i in range(layer_num):
            self.layers.append(BottleNeck(channel, channel, kernel=kernel, stride=1, exp=int(exp[i]), group=group))
...
class VoVNet_lite_normconv(nn.Module):
    def __init__(self,
                 channels=[3, 32, 32, 64, 128, 128, 256, 256],
                 frozen_stages=-1,
                 norm_eval=True):
        super(VoVNet_lite_normconv, self).__init__()
        self.frozen_stages = frozen_stages
        self.norm_eval = norm_eval
        self.upchannels = 32
        self.stem   = common_conv2d(channels[0], channels[1], kernel=3, padding=1, stride=2, relu=True)
        self.stage1 = nn.Sequential(BottleNeck(channels[1], channels[2], kernel=3, stride=1, exp=0.5, group=False)
                                    )

        self.stage2 = nn.Sequential(BottleNeck(channels[2], channels[3], kernel=3, stride=2, exp=0.5, group=False)
                                    )

        self.stage3 = nn.Sequential(BottleNeck(channels[3], channels[4], kernel=3, stride=2, exp=0.25, group=False),
                                    OSAModule(channels[4], kernel=3, exp=[0.25, 0.25, 0.25], layer_num=3, group=False)
                                    )

        self.stage4 = nn.Sequential(BottleNeck(channels[4], channels[5], kernel=3, stride=2, exp=6, group=False),
                                    OSAModule(channels[5], kernel=3,  exp=[0.25, 0.25, 0.25], layer_num=3, group=False)
                                    )

        self.stage5 = nn.Sequential(BottleNeck(channels[5], channels[6], kernel=3, stride=2, exp=6, group=False),
                                    OSAModule(channels[6], kernel=3, exp=[0.25, 0.25, 0.25], layer_num=3, group=False)
                                    )

        self.stage6 = nn.Sequential(BottleNeck(channels[6], channels[7], kernel=3, stride=2, exp=6, group=False),
                                    OSAModule(channels[7], kernel=3, exp=[0.25,0.25, 0.25], layer_num=3, group=False)
                                    )

        self.upconv_stage3 = BottleNeck(inplanes=channels[4], planes=self.upchannels, kernel=3, stride=1, exp=0.25, group=False)
        self.upconv_stage4 = BottleNeck(inplanes=channels[5], planes=self.upchannels, kernel=3, stride=1, exp=0.25, group=False)
        self.upconv_stage5 = BottleNeck(inplanes=channels[6], planes=self.upchannels, kernel=3, stride=1, exp=0.25, group=False)
        self.upconv_stage6 = BottleNeck(inplanes=channels[7], planes=self.upchannels, kernel=3, stride=1, exp=0.25, group=False)
        ...
        self.Upsample = Upsample_fuse([self.upconv_stage3,
                                       self.upconv_stage4,
                                       self.upconv_stage5,
                                       self.upconv_stage6],
                                      [self.deconv_stage6,
