...
class VFNetHeadDeploy(ATSSHead, FCOSHead):
    ...
    def forward(self, feats, onnx=False):
        ...
        return multi_apply(self.forward_single, feats, self.scales,
                           self.scales_refine, self.strides, self.reg_denoms, [onnx for _ in range(len(feats))])

    def forward_single(self, x, scale, scale_refine, stride, reg_denom, onnx=False):
        ...
        if self.bbox_norm_type == 'reg_denom':
            bbox_pred = scale(
                self.vfnet_reg(reg_feat_init)).exp() * reg_denom
        elif self.bbox_norm_type == 'stride':
            bbox_pred = scale(
                self.vfnet_reg(reg_feat_init)).exp() * stride
        else:
