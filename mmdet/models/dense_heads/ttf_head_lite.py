...
class TTFHead_lite(AnchorHead):
    ...
    def __init__(self,
        ...
        self.wh_out = nn.Conv2d(2*self.wh_planes,self.wh_planes, kernel_size=1, stride=1, padding=0)
        self.dynamic_wh_weight = nn.parameter.Parameter(torch.ones(1) * (16))
    ...
    def build_head(self, out_channel, conv_num=1, head_conv_plane=None):  #head_conv_plane=64 conv_num=2 out_channel=4
        ...
        inp = self.planes[-1]
        head_convs.append(ConvModule(inp, head_conv_plane, 1, padding=0, norm_cfg=dict(type='BN')))
        head_convs.append(ConvModule(head_conv_plane, head_conv_plane, 1, padding=0, groups=head_conv_plane , norm_cfg=dict(type='BN')))
        head_convs.append(ConvModule(head_conv_plane, head_conv_plane, 1, padding=0, groups=1, norm_cfg=dict(type='BN')))
    ...
    def forward(self, feats):
        ...
        hm = self.hm(x)
        # wh = self.wh(x) * self.wh_offset_base
        wh = self.wh(x)
        tmp = torch.cat((hm, hm, hm, hm, wh), dim=1)
        wh_out = F.relu(self.wh_out(tmp))*self.dynamic_wh_weight
    ...
    def FilerBbox(self, gt_boxes, gt_labels, feat_area):
        out_box = []
        out_labels = []
        for i in range(len(gt_boxes)):
            w = (gt_boxes[i][2]- gt_boxes[i][0])/self.down_ratio
            h = (gt_boxes[i][3] - gt_boxes[i][1])/self.down_ratio
            if w*h>(1/16*feat_area):
                out_box.append(gt_boxes[i])
                out_labels.append(gt_labels[i])
        return out_box, out_labels
